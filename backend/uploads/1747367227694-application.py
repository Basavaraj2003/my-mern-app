...
def classify(features):
    ...
    for i in [0, 2]:
        ...
        if not ipaddress.ip_address(ip).is_private:
            ...
            if country is not None and country not in ["ano", "unknown"]:
                img = (
                    ' <img src="static/images/blank.gif" class="flag flag-'
                    + country.lower()
                    + '" title="'
                    + country
                    + '">'
    ...
    flow_df.loc[len(flow_df)] = (
        [flow_count] + record + classification + proba_score + risk
    )
...
def flow_detail():
    flow_id = request.args.get(
        "flow_id", default=-1, type=int
    )  # /flow-detail?flow_id=x
    # print(flow_id)
    ...
    exp = explainer.explain_instance(
        choosen_instance[0], predict_fn_rf, num_features=6, top_labels=1
    )
    ...
    plot_div = plotly.offline.plot(
        {
            "data": [
                plotly.graph_objs.Bar(
                    x=col_n_largest[0].tolist(), y=err_n_largest[0].tolist()
                )
            ]
        },
    ...
    return render_template(
        ...
        ae_plot=plot_div,
        risk=risk,
    )  # titles=flow.columns.values, classifier='RF Classifier'
